...
*/
    ...
    if (step>0) {  
     for (i=0; i<nexp; i++) { 
      ...
      y += 1.0e-20 ;
      xexp[i] = log(2.0) / y ;
      ++xexp[i] ;  // add 1 to generation number
     }
    ...
    fprintf(ofile, "##fit: %s\n", iname) ;
    vmaxmin(xfit, lenxfit, &zmax, &zmin) ; 
    zden = fabs(zmax-zmin) ;
    znum = 0 ; 
    for (i=0; i<lenxfit; ++i)  { 
     ...
     y = xval[i] - xfit[i] ; 
     fprintf(ofile, "%12.6f ", y) ; 
     znum += y*y ; 
     fprintf(ofile, "\n") ;
    }
    znum = sqrt(znum/(double) lenxfit) ;
    printf("nrmsd: %9.3f\n", znum/zden) ;
//  printf("zzjunk: %12.6f %12.6f\n", znum, zden) ; 
  printf("##end of run\n") ;
  return 0 ;
}
void printhelp() 
